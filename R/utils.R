stats_count_uni <- function(champ_quali, max_modalites = NULL, lib_modalite_autre = NULL, choix_multiple = FALSE) {
  ...
  stats <- dplyr::mutate(stats, pct = scales::percent(.data$n / sum(stats$n), decimal.mark = ",", suffix = "\u202F%"))
...
stats_count_bi <- function(champ_quali, champ_x, identifiant = NULL, complet = FALSE) {
  ...
  stats <- dplyr::tibble(champ_x = champ_x, champ_quali = champ_quali) %>%
    ...
    dplyr::mutate(
      pos = cumsum(.data$n) - 0.5 * .data$n,
      pct = scales::percent(.data$n / sum(.data$n, na.rm = TRUE), decimal.mark = ",", suffix = "\u202F%")
    ) %>%
    dplyr::ungroup()
  ...
  if (complet == TRUE) {
    ...
    stats <- dplyr::mutate(stats, pct = scales::percent(.data$n / length(unique(identifiant)), decimal.mark = ",", suffix = "\u202F%")) %>%
...
pct_repondants <- function(repondants, total) {
  ...
  if (repondants != total) {
    n_repondants <- paste0("R\u00E9pondants : ", repondants, " (", scales::percent(repondants / total, decimal.mark = ",", suffix = "\u202F%"), ")")
  } else {
    n_repondants <- NULL
  }
